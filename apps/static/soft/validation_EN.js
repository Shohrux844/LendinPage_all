...
var feed = {
    submit: function (e, elem, field) {
        ...
        if (!name.length || name.length < 3) {
            e.preventDefault();
            return feed.errorMessage(form.find('[name=name]'), 'You did not fill the "Name"', field);
        }

        if (!phone.length || phone.length < 5) {
            e.preventDefault();
            return feed.errorMessage(form.find('[name=phone]'), 'You did not fill the "Phone"', field);
        }

        if (!rephone.test(phone)) {
            e.preventDefault();
            return feed.errorMessage(form.find('[name=phone]'), 'Incorrectly filled the "Phone"', field);
        }
